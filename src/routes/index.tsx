	...
	return (
		<>
			<section className="relative overflow-hidden px-6 py-24 text-center">
				<div
					aria-hidden
					className="pixelated absolute inset-0 -z-10 bg-[url(/decor/stone.png)] bg-size-[64px] opacity-[0.07] `mask-[linear-gradient(to_bottom,black,transparent)]"
				/>
			...
			<section id="tools" className="scroll-mt-16">
				<div className="grid sm:grid-cols-2 lg:grid-cols-3 `*:border-b *:border-border sm:[&>*:nth-child(odd)]:border-r lg:[&>*:nth-child(odd)]:border-r-0 lg:[&>*:not(:nth-child(3n))]:border-r">
					{TOOLS.map(({ to, title, description, icon: Icon }) => (
						<Link key={to} to={to} className="group flex flex-col gap-3 p-6 hover:bg-primary/10">
